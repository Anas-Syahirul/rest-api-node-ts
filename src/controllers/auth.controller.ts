...
export const registerUser = async (req: Request, res: Response) => {
  ...
  if (error) {
    logger.error(`ERR: auth - register = ${error.details[0].message}`, error.details[0].message)
...
export const createSession = async (req: Request, res: Response) => {
  ...
  if (error) {
    logger.error(`ERR: auth - create session = ${error.details[0].message}`, error.details[0].message)
    return res.status(422).send({ status: false, statusCode: 422, message: error.details[0].message })
  }
  try {
...
export const refreshSession = async (req: Request, res: Response) => {
  ...
  if (error) {
    logger.error(`ERR: auth - refresh session = ${error.details[0].message}`, error.details[0].message)
    return res.status(422).send({ status: false, statusCode: 422, message: error.details[0].message })
  }
  ...
  try {
    ...
    return res
      .status(200)
      .send({ status: true, statusCode: 200, message: 'Rfresh Sesion Success', data: { accessToken } })
  } catch (error: any) {
